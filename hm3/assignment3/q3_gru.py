...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        h = tf.zeros([tf.shape(x)[0], cell.state_size])
        predsAll, state = tf.nn.dynamic_rnn(cell, x, initial_state=h)
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients is False:
            ...
            param_list = [ g[1] for g in grad_all]
            grad_clip, _ = tf.clip_by_global_norm(grad_list, self.config.max_grad_norm)
            grad_new = zip(grad_clip, param_list)
            self.grad_norm = tf.global_norm(grad_clip)
            train_op = optimizer.apply_gradients(grad_new)
        ### END YOUR CODE
